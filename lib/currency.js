...
/**
 * SNKRDUNK USD→HKD rate (market rate ~7.78).
 */
export const SNKRDUNK_USD_TO_HKD = 7.78;

/**
 * Convert a SNKRDUNK price to HKD.
 * Priority: priceHkd (original HKD, no conversion) → priceUsd × 7.78 → priceJpy × live rate.
 */
 ...
 * For HKD: prefers priceHkd direct, then USD × 7.78.
 * For other currencies: falls back to JPY conversion.
 */
export function snkrdunkConvertPrice(priceUsd, priceJpy, toCurrency, priceHkd) {
    if (toCurrency === 'HKD') {
        if (priceHkd && priceHkd > 0) return priceHkd;
        if (priceUsd > 0) return priceUsd * SNKRDUNK_USD_TO_HKD;
    }
