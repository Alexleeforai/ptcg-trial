...
/**
 * SNKRDUNK USD→HKD (align with market / Google Finance ~7.83–7.84).
 */
export const SNKRDUNK_USD_TO_HKD = 7.835;

/**
 * Convert a SNKRDUNK price to HKD.
 * Uses the lowest plausible value among: real USD×rate, stored HKD, JPY×live rate.
 * Ignores "synthetic" USD that was back-computed from HKD (legacy rows where USD×rate ≈ priceHkd).
 */
export function snkrdunkToHkd(priceUsd, priceJpy, jpyHkdRate, priceHkd) {
    const rate = SNKRDUNK_USD_TO_HKD;
    const parts = [];
    if (priceUsd && priceUsd > 0) {
        const fromUsd = Math.round(priceUsd * rate);
        const h = priceHkd && priceHkd > 0 ? Math.round(priceHkd) : 0;
        const diff = h > 0 ? Math.abs(fromUsd - h) : Infinity;
        const synthetic = h > 0 && diff < Math.max(80, h * 0.04);
        if (!synthetic) parts.push(fromUsd);
    }
    if (priceHkd && priceHkd > 0) parts.push(Math.round(priceHkd));
    if (priceJpy && priceJpy > 0) parts.push(Math.round(priceJpy * (jpyHkdRate || DEFAULT_RATE)));
    if (parts.length === 0) return 0;
    return Math.min(...parts);
}

/**
 * Convert a SNKRDUNK price to any currency.
 * For HKD: same min logic as snkrdunkToHkd.
 */
export function snkrdunkConvertPrice(priceUsd, priceJpy, toCurrency, priceHkd, jpyHkdRate) {
    if (toCurrency === 'HKD') return snkrdunkToHkd(priceUsd, priceJpy, jpyHkdRate || DEFAULT_RATE, priceHkd);
    if (toCurrency === 'USD' && priceUsd > 0) return priceUsd;
    return convertPrice(priceJpy || 0, 'JPY', toCurrency);
}
