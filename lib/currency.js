...
/**
 * SNKRDUNK's implied USD→HKD rate (observed from their HK display prices).
 * Using this avoids the USD→JPY→HKD round-trip which undershoots by ~2-5%.
 */
export const SNKRDUNK_USD_TO_HKD = 7.50;

/**
 * Convert a SNKRDUNK price to HKD.
 * Prefers the stored USD source amount (× 7.50) over JPY (× live rate).
 */
export function snkrdunkToHkd(priceUsd, priceJpy, jpyHkdRate) {
 ...
 * For HKD: prefers USD source × 7.50 to avoid USD→JPY→HKD round-trip error.
 * For other currencies: falls back to JPY conversion.
 */
export function snkrdunkConvertPrice(priceUsd, priceJpy, toCurrency) {
    if (toCurrency === 'HKD' && priceUsd > 0) return priceUsd * SNKRDUNK_USD_TO_HKD;
