 ...
 * Priority: stored priceHkd (computed from actual listing currency) → priceUsd×rate → priceJpy×rate.
 * priceHkd is authoritative because it was computed in snkrdunk.js from the real listing currency
 * (USD or HKD), avoiding double-conversion errors from JPY live rates.
 */
export function snkrdunkToHkd(priceUsd, priceJpy, jpyHkdRate, priceHkd) {
    if (priceHkd && priceHkd > 0) return Math.round(priceHkd);
    if (priceUsd && priceUsd > 0) return Math.round(priceUsd * SNKRDUNK_USD_TO_HKD);
    if (priceJpy && priceJpy > 0) return Math.round(priceJpy * (jpyHkdRate || DEFAULT_RATE));
    return 0;
}
