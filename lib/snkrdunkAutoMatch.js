...
export function parseCardIndexFromDb(card) {
    ...
    // e.g. "Charizard EX #201" — number embedded at end of name
    const m3 = (card.name || '').match(/#\s*0*(\d+)\s*$/i);
    if (m3) return String(parseInt(m3[1], 10)).padStart(3, '0');
    // e.g. "4/102" pattern in name
...
/**
 * @param {{ setCode?: string, number?: string, name?: string }} card
 * @returns {Promise<{ match: { id: number, name: string } | null, skipReason: string | null }>}
 *   skipReason: 'no_keyword' | 'no_card_index' | 'no_snk_results' | 'no_bracket_match' | 'multi_candidates' | null (matched)
 */
export async function autoMatchOneCard(card) {
    const kw = pickKeywordFromCardName(card.name);
    if (!kw || kw.length < 3) return { match: null, skipReason: 'no_keyword' };

    const idx = parseCardIndexFromDb(card);
    if (!idx) return { match: null, skipReason: 'no_card_index' };

    const listings = await searchSnkrdunkTradingCards(kw);
    if (!listings || listings.length === 0) return { match: null, skipReason: 'no_snk_results' };

    const dbCode = normalizeSetCode(card.setCode).toLowerCase();

    // Check if any result has a valid Pokemon-style bracket [CODE NUM/DENOM]
    const hasPokemonBracket = listings.some((item) => parseBracketFromSnkName(item.name || '') !== null);
    if (!hasPokemonBracket) {
        // SNKRDUNK returned results but none have Pokemon bracket format —
        // likely irrelevant products (One Piece, Gundam etc.) due to API limitations
        return { match: null, skipReason: 'no_pokemon_bracket' };
    }
    ...
    if (strong.length === 0) return { match: null, skipReason: 'no_bracket_match' };
    if (strong.length === 1) return { match: strong[0], skipReason: null };
    return { match: null, skipReason: 'multi_candidates' };
}
