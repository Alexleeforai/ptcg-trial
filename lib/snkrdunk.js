...
export async function fetchSnkrdunkTradingCardQuote(productId) {
    ...
    // Try page HTML first for base price
    const parsed = parseTradingCardHtml(html);
    let minPrice = parsed?.minPrice ?? 0;
    let currency = parsed?.currency ?? 'HKD';
    let source = 'page';

    if (minPrice <= 0) {
        const fromFormat = parseMinPriceFormatHkd(html);
        if (fromFormat != null && fromFormat.amount > 0) {
    ...
    // Fall back to listings (overall minimum across all conditions)
