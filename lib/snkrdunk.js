...
export async function fetchSnkrdunkTradingCardQuote(productId) {
    ...
    // Also keep USD for backward compat
    const priceUsd = currency === 'USD' ? minPrice : (currency === 'HKD' ? Math.round(minPrice / 7.78) : null);
        ...
        try {
            ...
            for (const row of items) {
                ...
                if (!byCondition[cond]) {
                    ...
                    if (listingToHkd(p) < listingToHkd(byCondition[cond])) byCondition[cond] = p;
                }
            }
            return byCondition;
        } catch {
...
function listingToHkd(p) {
    ...
    if (p.currency === 'USD') return p.amount * 7.78;
