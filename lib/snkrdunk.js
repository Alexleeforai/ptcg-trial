...
export async function fetchSnkrdunkTradingCardQuote(productId) {
    ...
    // ── Raw price: always prefer Grade A listing (avoids showing damaged/C-grade price as "Raw") ──
    // Priority: Grade A listing → Grade B → page HTML minPrice → minPriceFormat → overall min
    let minPrice = 0;
    let currency = 'HKD';
    ...
    // Fall back to page HTML if no Grade A/B listing found
    if (minPrice <= 0) {
    ...
    // Last resort: overall minimum across all conditions
    ...
    // Store original USD amount for accurate HKD display (avoids USD→JPY→HKD round-trip error)
    const priceUsd = currency === 'USD' ? minPrice : (currency === 'HKD' ? minPrice / 7.5 : null);
...
async function fetchAllListingsByCondition(tradingCardId) {
    // Use price-ascending sort (sortType=2) and Japanese Accept-Language to surface JP-marketplace
    // listings that are priced in USD but only visible via JA locale headers.
    const urls = [
        `https://snkrdunk.com/en/v1/trading-cards/${tradingCardId}/used-listings?perPage=50&page=1&sortType=2&isOnlyOnSale=true`,
        `https://snkrdunk.com/en/v1/trading-cards/${tradingCardId}/used-listings?perPage=50&page=1&sortType=2&isOnlyOnSale=false`
    ];
    ...
    for (const url of urls) {
        try {
            ...
            // Group by condition, keep cheapest per group
            const byCondition = {};
            for (const row of items) {
                ...
                if (!byCondition[cond]) {
                    ...
                    const jpy = Math.round(convertPrice(p.amount, p.currency, 'JPY'));
                    const bestJpy = Math.round(convertPrice(byCondition[cond].amount, byCondition[cond].currency, 'JPY'));
                    if (jpy < bestJpy) byCondition[cond] = p;
                }
            }
            return byCondition;
        } catch {
...
/**
 * Find the cheapest price across all conditions (JPY basis).
 */
function findOverallMin(byCondition) {
    ...
    for (const p of Object.values(byCondition)) {
        ...
        const jpy = Math.round(convertPrice(p.amount, p.currency, 'JPY'));
        const bestJpy = Math.round(convertPrice(best.amount, best.currency, 'JPY'));
        if (jpy < bestJpy) best = p;
    }
    return best;
}
