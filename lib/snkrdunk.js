...
export async function fetchSnkrdunkTradingCardQuote(productId) {
    ...
    // ── Raw price: Grade A/B/S listing vs page HTML headline (minPrice / minPriceFormat) ──
    // SNKRDUNK often shows "US $358~" on the page while the used-listings API first page
    // may only return a pricier HKD seller — compare all in HKD and take the cheapest.
    let listingCand = null;
    ...
    const htmlCands = [];
    const parsedPage = parseTradingCardHtml(html);
    if (parsedPage?.minPrice > 0) {
        htmlCands.push({
            amount: parsedPage.minPrice,
            currency: parsedPage.currency,
            source: 'page',
        });
    }
    ...
    if (fromFormat?.amount > 0) {
        htmlCands.push({
            amount: fromFormat.amount,
            currency: fromFormat.currency,
            source: 'minPriceFormat',
        });
    }

    const cands = [...(listingCand ? [listingCand] : []), ...htmlCands];
    let best = null;
    if (cands.length > 0) {
        best = cands[0];
        for (const c of cands.slice(1)) {
            if (listingToHkd(c) < listingToHkd(best)) best = c;
        }
    }

    if (!best && listingsByCondition) {
...
async function fetchAllListingsByCondition(tradingCardId) {
    ...
    // Group by condition, keep cheapest per group (compare in HKD to avoid
    // picking an expensive HKD listing over a cheaper USD listing)
    const byCondition = {};
    for (const row of allRows) {
        ...
        if (!byCondition[cond]) {
            byCondition[cond] = p;
        } else if (listingToHkd(p) < listingToHkd(byCondition[cond])) {
            byCondition[cond] = p;
        }
    }
    return byCondition;
