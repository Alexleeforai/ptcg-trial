...
export async function findCards(query, type = 'all') {
    ...
    if (type === 'setCode') {
        ...
        // 'all' — setCode 必須成粒代號完全一致（唔分大小寫），唔做 substring（避免 xxm2a、xm2a 等）
        const qstr = query instanceof RegExp ? '' : String(query).trim();
        const setCodeExact =
            qstr.length > 0 ? { setCode: new RegExp(`^${escapeRegexForDb(qstr)}$`, 'i') } : { setCode: { $regex: regex } };
        ...
        searchConditions = [
            ...
            { set: { $regex: regex } },
            { setId: { $regex: regex } },
            query instanceof RegExp ? { setCode: { $regex: regex } } : setCodeExact,
            { number: { $regex: regex } }
        ];
