...
import { getCategoryFromCard, POKEMON_NAMES } from './pokedex';
...
// 9. Get Browse Categories
export async function getBrowseCategories() {
    await db();
    // Get all SINGLE cards only
    // Only fetch necessary fields to reduce memory
    const cards = await Card.find({
        $or: [{ cardType: 'single' }, { cardType: { $exists: false } }] // Assume singles if missing type for legacy
    }).select('name image').lean();

    const categories = {
        trainers: { name: 'Trainers', count: 0, image: '' },
        items: { name: 'Items', count: 0, image: '' },
        pokemon: {} // Map: "Pikachu" -> { count: 0, image: '...' }
    };

    cards.forEach(card => {
        const cat = getCategoryFromCard(card);
        if (cat === 'Trainers') {
            ...
            // It's a Pokemon
            if (!categories.pokemon[cat]) {
                categories.pokemon[cat] = { name: cat, count: 0, image: '' };
            }
            categories.pokemon[cat].count++;
            // Prioritize image if missing
            if (!categories.pokemon[cat].image && card.image) {
                categories.pokemon[cat].image = card.image;
            }
        }
    });

    // Convert pokemon map to array and sort A-Z
