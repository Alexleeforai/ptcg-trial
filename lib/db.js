...
export async function findCards(query, type = 'all') {
    ...
    if (!hasCJK(qstr) && await queryMatchesSetCode(qLower)) {
        ...
        const cards = await Card.find({ setCode: { $regex: exactRegex } }).limit(500).lean();
        return toPlainObject(cards);
    }
