...
/**
 * Fetches the full list of Pokemon card sets from PriceCharting.
 * @returns {Promise<Array<{name: string, url: string, id: string}>>}
 */
export async function getAllSetsFromPC() {
...
function parseCardsFromHTML($, set) {
    ...
    $('#games_table tbody tr').each((i, el) => {
        ...
        // Prices
        const priceLoose = parsePrice(row.find('.price.used_price .js-price').text()) || parsePrice(row.find('.price.used_price').text());
        const priceCIB = parsePrice(row.find('.price.cib_price .js-price').text()) || parsePrice(row.find('.price.cib_price').text());
        const priceNew = parsePrice(row.find('.price.new_price .js-price').text()) || parsePrice(row.find('.price.new_price').text());

        // Image
        const img = row.find('td.image img');
        let image = img.attr('src') || img.attr('data-src');
        if (image) {
            // Upgrade from /tiny/ or /small/ to /large/ (which corresponds to /240.jpg usually)
            image = image.replace('/tiny/', '/large/').replace('/small/', '/large/');
        }

        if (id && name) {
            cards.push({
                id: `pc-${set.id}-${id}`, // Prefix and Set ID to avoid collision and match old DB schema
                name: name,
                set: set.name,
                setId: set.id,
                priceRaw: priceLoose || 0,
                priceGrade9: priceCIB || 0,
                pricePSA10: priceNew || 0,
                image: image || null,
                currency: 'USD',
