...
export default function TosPage() {
    ...
    useEffect(() => {
        fetch('/api/legal?key=tos').then(r => r.json()).then(d => { setContent(d.content || ''); setLoading(false); }).catch(() => setLoading(false));
    }, []);
    return <LegalPage title="服務條款 (Terms of Service)" content={content} loading={loading} />;
}
