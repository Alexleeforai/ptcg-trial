...
export default function PrivacyPage() {
    ...
    useEffect(() => {
        fetch('/api/legal?key=privacy').then(r => r.json()).then(d => { setContent(d.content || ''); setLoading(false); }).catch(() => setLoading(false));
    }, []);
    return <LegalPage title="私隱政策 (Privacy Policy)" content={content} loading={loading} />;
}
